...
<body class="gc-documentation
  <?cs if:(devguide||codeqa||reference||devtools||develop) ?>develop<?cs if:usage ?> usage<?cs /if ?><?cs
  elif:about ?>about<?cs
  elif:getit ?>getit<?cs
  elif:foss ?>foss<?cs
...
<div <?cs if:fullpage
?>class="fullpage"<?cs elif:devtools||about||getit
?>class="col-13" id="doc-col"<?cs else 
?>class="col-12" id="doc-col"<?cs /if ?> >

<?cs if:(usage||devguide) ?><?cs # header logic for docs that provide previous/next buttons ?>
  <?cs if:header.hide ?>
  <?cs else ?>
    ...
    <?cs call:tag_list(root.descr) ?>
      ...
      <div class="content-footer <?cs 
        ...
        <div class="paging-links layout-content-col col-4">
          <?cs if:(usage||devguide) && !page.landing && !footer.hide ?>
            <a href="#" class="prev-page-link hide">Previous</a>
            <a href="#" class="next-page-link hide">Next</a>
          <?cs /if ?>
        </div>
